//! Uniswap V3 concentrated-liquidity primitives.
//!
//! Vendored from `dexloom/loom` (`crates/defi/uniswap-v3-math`, MIT) and
//! ported to the workspace's alloy 2.x. Only `alloy::primitives` math types
//! are used — the upstream provider-coupled `..._from_provider` RPC variant
//! was dropped so this module stays pure (no I/O, no async). Tick data is
//! supplied by the caller via the [`tick_provider::TickProvider`] trait.
//!
