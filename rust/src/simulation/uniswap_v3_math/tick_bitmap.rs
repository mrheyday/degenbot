use crate::simulation::uniswap_v3_math::tick_provider::TickProvider;
use crate::simulation::uniswap_v3_math::U256_1;
use crate::simulation::uniswap_v3_math::{bit_math, error::UniswapV3MathError};
...
#[cfg(test)]
// Vendored upstream (loom) test suite — kept verbatim. Upstream's assertion
// style trips `bool_assert_comparison` / `needless_borrow`; allowed here so a
// future re-vendor stays a clean diff.
...
mod test {
    ...
    pub fn initialized<T: TickProvider>(tick: i32, tick_provider: &T) -> eyre::Result<bool> {
        let (next, initialized) =
            next_initialized_tick_within_one_word(tick_provider, tick, 1, true)?;
        if next == tick {
            Ok(initialized)
        } else {
            Ok(false)
