...
def _build_v4_v2_v2_world(seed: int):
    owner = mk_addr(0xD400 + seed)
    executor_addr = mk_addr(0xD500 + seed)
    ...
    pool1_addr = mk_addr(0xD700 + seed)
    pool2_addr = mk_addr(0xD800 + seed)
    pool3_addr = mk_addr(0xD900 + seed)

    # Triangle: V4 outer -> V2 middle -> V2 inner
    # pool1: WETH -> TOKEN_A
    # pool2: TOKEN_A -> TOKEN_B
    # pool3: TOKEN_B -> WETH
    pool1 = MockV2Pair(weth, token_a, pool1_addr, swap_fee=0)
    pool2 = MockV2Pair(token_a, token_b, pool2_addr, swap_fee=0)
    pool3 = MockV4Pool(token_b, weth, pool3_addr)

    weth_in = 1_000_001_000_000
    token_a_out = 1_000_000_000_000
    token_b_out = 900_000_000_000
    weth_out = 1_200_000_000_000
    ...
    pool1.amount_in = weth_in
    pool1.amount_out = token_a_out
    pool2.amount_in = token_a_out
    pool2.amount_out = token_b_out
...
def test_v4_v2_v2_nested_callback_chain_executes_and_clears_state() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v2_v2_world(seed)
    ...
    gas_used = executor.execute_v4_v2_v2_nested_arbitrage(
        pool3,
        pools,
        payload,
        seed,
...
def test_v4_v2_v2_nested_callback_chain_order_security() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v2_v2_world(seed)
    pool1, pool2, _ = tuple(pools.values())
    ...
    with pytest.raises(AssertionError):
        executor.execute_v4_v2_v2_nested_arbitrage(
        pool3,
        pools,
        payload,
        seed,
...
def test_v4_v2_v2_nested_arbitrage_metric() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v2_v2_world(seed)
    pool1, pool2, _ = tuple(pools.values())
    expected_chain = [pool3.address, pool2.address, pool1.address]
    ...
    print("\n  V4→V2→V2 nested arbitrage metric")
