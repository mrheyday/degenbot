...
def _build_v4_v3_v2_world(seed: int):
    owner = mk_addr(0xC400 + seed)
    executor_addr = mk_addr(0xC500 + seed)
    ...
    pool1_addr = mk_addr(0xC700 + seed)
    pool2_addr = mk_addr(0xC800 + seed)
    pool3_addr = mk_addr(0xC900 + seed)

    # Triangle: V4 outer -> V3 middle -> V2 inner
    # pool3: TOKEN_B -> WETH
    ...
    token_a_out = 1_100_000_000_000_000
    weth_in = 1_103_000_000_000_000
    token_b_out = 1_250_000_000_000_000
    weth_out = 1_500_000_000_000_000
    ...
    # Seed V3/V4 output liquidity; V2 uses sync to initialize its reserve view.
    pool1.sync()

    # Pool configs
    pool2.set_next_swap(token_a_out, token_b_out, True)
    pool3.set_next_swap(token_b_out, weth_out, True)
    # For the innermost V2 leg we need to consume tokenA_out; this is enforced in swaps.
...
def test_v4_v3_v2_nested_callback_chain_executes_and_clears_state() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v3_v2_world(seed)
...
def test_v4_v3_v2_nested_callback_chain_order_security() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v3_v2_world(seed)
    pool1, pool2, _ = tuple(pools.values())
...
def test_v4_v3_v2_nested_arbitrage_metric() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v3_v2_world(seed)
    ...
    print("\n  V4→V3→V2 nested arbitrage metric")
