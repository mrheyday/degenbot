...
def _build_v2_v4_v4_world(seed: int):
    owner = mk_addr(0x7400 + seed)
    executor_addr = mk_addr(0x7500 + seed)
    ...
    # Triangle: V4 (pool1) -> V4 (pool2) -> V2 (pool3)
    # pool3 outputs WETH directly to executor and routes callbacks through V4 chain.
    pool1 = MockV4Pool(weth, token_a, pool1_addr)
    pool2 = MockV4Pool(token_a, token_b, pool2_addr)
    pool3 = MockV2Pair(token_b, weth, pool3_addr)

    weth_in = 1_200_000_000_000_000
    token_a_out = 1_250_000_000_000_000
    token_b_out = 1_300_000_000_000_000
    weth_out = 1_700_000_000_000_000

    # Seed reserves/lends for clean callback reconciliation.
    ...
    # Configure V4 leg swap expectations.
    pool1.set_next_swap(weth_in, token_a_out, True)
    pool2.set_next_swap(token_a_out, token_b_out, True)

    # V2 outer leg output is WETH on token1 side.
    pool3.reserve0 = 0
    pool3.reserve1 = weth_out
...
def test_v2_v4_v4_nested_callback_chain_executes_and_clears_state() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v2_v4_v4_world(seed)
    ...
    gas_used = executor.execute_v2_v4_v4_nested_arbitrage(
        pool3,
        pools,
        payload,
        seed,
...
def test_v2_v4_v4_nested_callback_chain_order_security() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, _, output_amount = _build_v2_v4_v4_world(seed)
    pool1, pool2, _ = tuple(pools.values())
    ...
    with pytest.raises(AssertionError):
        executor.execute_v2_v4_v4_nested_arbitrage(
            pool3,
            pools,
            payload,
            seed,
...
def test_v2_v4_v4_nested_arbitrage_metric() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v2_v4_v4_world(seed)
    pool1, pool2, _ = tuple(pools.values())
    ...
    print("\n  V2→V4→V4 nested arbitrage metric")
