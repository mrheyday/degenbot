...
def _build_v4_v2_v4_world(seed: int):
    owner = mk_addr(0xE400 + seed)
    executor_addr = mk_addr(0xE500 + seed)
    ...
    pool1_addr = mk_addr(0xE700 + seed)
    pool2_addr = mk_addr(0xE800 + seed)
    pool3_addr = mk_addr(0xE900 + seed)

    # Triangle: V4 outer -> V2 middle -> V4 inner
    # pool3: TOKEN_B -> WETH
    ...
    pay_amount = 1_000_000_000_000
    token_a_out = 1_250_000_000_000
    token_b_out = 1_000_000_000_000
    weth_out = 1_300_000_000_000
    ...
    # Configure leg swaps.
    pool1.set_next_swap(pay_amount, token_a_out, True)
...
def test_v4_v2_v4_nested_callback_chain_executes_and_clears_state() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v2_v4_world(seed)
    ...
    gas_used = executor.execute_v4_v2_v4_nested_arbitrage(
        pool3,
        pools,
        payload,
        seed,
...
def test_v4_v2_v4_nested_callback_chain_order_security() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v2_v4_world(seed)
    pool1, pool2, _ = tuple(pools.values())
    ...
    with pytest.raises(AssertionError):
        executor.execute_v4_v2_v4_nested_arbitrage(
            pool3,
            pools,
            payload,
            seed,
...
def test_v4_v2_v4_nested_arbitrage_metric() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v2_v4_world(seed)
    pool1, pool2, _ = tuple(pools.values())
    expected_chain = [pool3.address, pool2.address, pool1.address]
    ...
    print("\n  V4→V2→V4 nested arbitrage metric")
