...
def _build_v4_v3_v3_world(seed: int):
    owner = mk_addr(0xA400 + seed)
    executor_addr = mk_addr(0xA500 + seed)
    ...
    pool1_addr = mk_addr(0xAD00 + seed)
    pool2_addr = mk_addr(0xAE00 + seed)
    pool3_addr = mk_addr(0xAF00 + seed)

    pool1 = MockV3Pool(weth, token_a, pool1_addr)
    ...
    # Seed liquidity to each pool so expected output is always available.
    ...
    # Inner V3 pair1: WETH -> Token A (output on token1)
    pool1.set_next_swap(weth_in, token_a_out, True)
    # Inner V3 pair2: Token A -> Token B
    pool2.set_next_swap(token_a_out, token_b_out, True)
    # Outer V4: Token B -> WETH
...
def test_v4_v3_v3_nested_callback_chain_executes_and_clears_state() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v3_v3_world(seed)
    ...
    gas_used = executor.execute_v4_v3_v3_nested_arbitrage(
        pool3,
        pools,
        payload,
        seed,
...
def test_v4_v3_v3_nested_callback_chain_order_security() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, _, output_amount = _build_v4_v3_v3_world(seed)
    pool1, pool2, _ = tuple(pools.values())
    ...
    with pytest.raises(AssertionError):
        executor.execute_v4_v3_v3_nested_arbitrage(
            pool3,
            pools,
            payload,
            seed,
...
def test_v4_v3_v3_nested_arbitrage_metric() -> None:
    seed = _seed_from_env()
    executor, pool3, pools, payload, pay_amount, output_amount = _build_v4_v3_v3_world(seed)
    pool1, pool2, _ = tuple(pools.values())
    ...
    print("\n  V4→V3→V3 nested arbitrage metric")
