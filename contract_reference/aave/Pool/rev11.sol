...
library UserConfiguration {
        ...
        returns (uint256)
    {
        unchecked {
            ...
            if (firstAssetPosition == 0) {
                return 0;
            }
            uint256 leadingZeroes;
            assembly ("memory-safe") {
                leadingZeroes := clz(firstAssetPosition)
            }
            return (255 - leadingZeroes) >> 1;
        }
    }
}
...
library BorrowLogic {
    ...
    function executeRepay(
        ...
        if (params.useATokens) {
            ...
            // params.user is set by Pool.repay() to _msgSender(); this pull cannot target an unrelated owner.
...
library ValidationLogic {
    ...
    function validateFlashloan(
        ...
        uint256 assetsLength = assets.length;
        require(assetsLength == amounts.length, Errors.InconsistentFlashloanParams());
        for (uint256 i = 0; i < assetsLength;) {
            for (uint256 j = i + 1; j < assetsLength;) {
...
library SupplyLogic {
    ...
    function executeSupply(
        ...
        // params.user is set by Pool.supply() to _msgSender(); this pull cannot target an unrelated owner.
        // slither-disable-next-line arbitrary-send-erc20
 ...
 *
 * NOTE: This flattened Pool uses the base {Context}, so `_msgSender()` is always `msg.sender`
 * and there is no ERC2771 suffix to propagate.
 */
...
library LiquidationLogic {
    ...
    function executeLiquidationCall(
        ...
        // params.liquidator is set by Pool.liquidationCall() to _msgSender().
        // slither-disable-next-line arbitrary-send-erc20
 ...
 */
library SafeCast {
    // SafeCast guards each narrowing conversion immediately before the cast;
    // Forge lint does not currently propagate those bounds across the cast.
    /**
...
library Address {
    ...
    /**
     * @dev Verifies that a low level call to smart-contract was successful.
     */
...
library FlashLoanLogic {
    ...
    function executeFlashLoan(
        ...
        uint256 assetsLength = params.assets.length;
        vars.totalPremiums = new uint256[](assetsLength);
    ...
    function _handleFlashLoanRepayment(
        ...
        // The flash-loan receiver returned success from executeOperation and must approve Pool repayment.
        // slither-disable-next-line arbitrary-send-erc20
...
abstract contract Pool is VersionedInitializable, PoolStorage, IPool, Multicall {
    ...
    /// @notice Address provider key used for backward-compatible stable debt mock lookups.
    bytes32 internal constant MOCK_STABLE_DEBT = "MOCK_STABLE_DEBT";
    ...
    function repayWithPermit(
        ...
        return repay(asset, amount, interestRateMode, onBehalfOf);
    }
        ...
        returns (DataTypes.ReserveDataLegacy memory res)
    {
        ...
        res.stableDebtTokenAddress = ADDRESSES_PROVIDER.getAddress(MOCK_STABLE_DEBT);
    }
 ...
 */
// VersionedInitializable sets lastInitializedRevision in the constructor, locking the implementation while leaving proxy initialization available.
