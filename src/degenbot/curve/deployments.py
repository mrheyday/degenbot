from degenbot.checksum_cache import get_checksum_address

CURVE_V1_REGISTRY_ADDRESS = get_checksum_address("0x90E00ACe148ca3b23Ac1bC8C240C2a7Dd9c2d7f5")
CURVE_V1_FACTORY_ADDRESS = get_checksum_address("0x127db66E7F0b16470Bec194d0f496F9Fa065d0A9")
CURVE_V1_METAREGISTRY_ADDRESS = get_checksum_address("0xF98B45FA17DE75FB1aD0e7aFD971b0ca00e379fC")

ARBITRUM_CURVE_STABLESWAP_NG_FACTORY = get_checksum_address(
    "0x9AF14D26075f142eb3F292D5065EB3faa646167b"
)
ARBITRUM_CURVE_STABLESWAP_NG_VIEWS = get_checksum_address(
    "0x3BbA971980A721C7A33cEF62cE01c0d744F26e95"
)
ARBITRUM_CURVE_STABLESWAP_NG_MATH = get_checksum_address(
    "0xD4a8bd4d59d65869E99f20b642023a5015619B34"
)
ARBITRUM_CURVE_STABLESWAP_NG_PLAIN_IMPL = get_checksum_address(
    "0xf6841C27fe35ED7069189aFD5b81513578AFD7FF"
)
ARBITRUM_CURVE_STABLESWAP_NG_META_IMPL = get_checksum_address(
    "0xFf02cBD91F57A778Bab7218DA562594a680B8B61"
)

BROKEN_CURVE_V1_POOLS = frozenset(
    get_checksum_address(pool_address)
    for pool_address in (
        "0x110cc323ca53d622469EdD217387E2E6B33F1dF5",
        "0x1F71f05CF491595652378Fe94B7820344A551B8E",
        "0x28B0Cf1baFB707F2c6826d10caf6DD901a6540C5",
        "0x3685646651FCcC80e7CCE7Ee24c5f47Ed9b434ac",
        "0x4029f7DcBdF6059ed80DA6856526E7510D64fA21",
        "0x69ACcb968B19a53790f43e57558F5E443A91aF22",
        "0x84997FAFC913f1613F51Bb0E2b5854222900514B",
        "0x9CA41a2DaB3CEE15308998868ca644e2e3be5C59",
        "0xA77d09743F77052950C4eb4e6547E9665299BecD",
        "0xD652c40fBb3f06d6B58Cb9aa9CFF063eE63d465D",
    )
)
